...
CONFIGURACION --> CONFIGURACION : A [count < max_count] /\n  count += 1\n  display.show(FILL[count])  \n  \n B [count > min_count] /\n  count -= 1\n  display.show(FILL[count])

ARMADO --> ARMADO : Timeout [count > 0] /\n  count -= 1\n  display.show(FILL[count])\n  myTimer.start()\n \n    A / (ignored)  -   B / (ignored)  -  S / (ignored)
ARMADO --> TERMINADO : Timeout [count == 0] /\n  display.show(FILL[0])
TERMINADO : entry /\n  display.show(Image.SKULL)\n  music.play(music.WAWAWAWAA, wait=False)

TERMINADO ---> CONFIGURACION : A /\n  count = 20\n  display.show(FILL[count])
TERMINADO -> TERMINADO : S / (ignored)  \nB / (ignored)

@enduml
 ```

<img width="606" height="665" alt="image" src="https://github.com/user-attachments/assets/509f1e50-3be9-4ef5-8588-cbb3aef9dd01" />
...
class Task:
    ...
    def estado_armado(self, ev):
        if ev == "ENTRY":
            ...
            # si hago shake estando armado podría rearmar o ignorar; aquí ignoramos
            pass
